...
| [DOC-001](job_scheduler.md#doc-001) | Use the installed `docent-python==0.1.77`, a real spawn/exec child, and a local HTTP server. For representative non-ASCII records, compare canonical local JSON with child reconstruction and compare uncompressed pinned-SDK payload bytes before and after the boundary while varying gzip metadata/HTTP framing. Inspect the real child PID/group, fixed FDs, stdio, and cleared environment; seed ambient proxy, custom CA, `.netrc`, Docent config, preload, and `PYTHONPATH` inputs and prove none is discovered. Under the module spawn lock prove observed unrelated inheritable FDs close and ordinary Python FDs are non-inheritable, retaining the approved trusted same-process-code limitation. Seed parent signal/thread/Torch/tqdm state and prove it is unchanged; separately orphan a child and observe its same-absolute alarm. Run one end-to-end deadline case with a `SIGTERM`-ignoring helper and known in-group descendant, measuring the at-most-250-millisecond drain, group `SIGKILL`, direct-child reap, known-descendant absence, possible network activity during drain, and no grace. Representative real-server cases cover 10/120, one-send redirect/non-2xx refusal, immediate job IDs, 100-ID slices, and all census status classes. Representative strict-protocol cases cover malformed frames, validated-ID versus null windows, fresh interrupt/SystemExit control flow, broken stderr, and locally evidenced success. | Canonical scientific JSON survives reconstruction and uncompressed pinned-SDK payload bytes match, without equating local JSONL to Unicode-escaped SDK JSON or variable gzip/HTTP bytes. The FD trust rule and proxy/custom-CA/`.netrc`/Docent-config exclusions hold as Ethan-reviewed; the remaining environment isolation retains its existing provenance. Parent state is unchanged and the child alarm is orphan defense. The representative deadline proves TERM/drain/KILL/reap stays inside five minutes, reaps the direct child, removes the known descendant, and provides no socket cutoff claim. Existing collection naming, 10/120, one-send/no-redirect/no-adoption, 2xx, job-ID/census, receipt/last-ID/null, control-flow, broken-stderr, and local-evidence behavior is independently observed. These representative proofs do not relax the operative clause or claim an exhaustive failure matrix or arbitrary-descendant census. |
