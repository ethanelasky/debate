...
| D-017 | Claude delegated originally; Ethan subsequently reviewed the worked example and approved the described after behavior | Each launch uses a namespace-named external Docent collection while the scientific `AgentRun` payload remains unchanged. Under D-025, exact canonical scientific JSON bytes cross local file-to-child reconstruction, and the child matches the pre-boundary pinned SDK's uncompressed JSON payload bytes at the batching/transport-serialization boundary. SDK Unicode escaping means that payload need not equal local newline JSONL; gzip metadata and HTTP framing may also vary. Ethan's later review removes the earlier “substance not reviewed/reversible” status for this Docent behavior. | [DOC-001](job_scheduler.md#doc-001) | `L1081-L1166` |
...
|---|---|---|---|---|
| D-022 | Codex recommended; Ethan reviewed and approved; **mechanism superseded by D-025** | The former design put the hard deadline's alarm in the main process and therefore required the main Python thread, no other live Python thread, no active `ITIMER_REAL`, and `SIGALRM` neither blocked nor pending. The conversation sequence corrects the immutable source's colloquial “directly approved” label: Codex recommended the mechanism and Ethan approved it. D-025 replaces this mechanism; D-018's five-minute budget and timeout substance remain operative. | Historical only; current behavior is [DOC-001](job_scheduler.md#doc-001) | `L1081-L1166` plus the governing conversation |
...
### 2026-08-18 — dedicated Docent upload child
