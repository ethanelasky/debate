...
def test_family_tasks_carry_question(name, tmp_path):
    ...
    elif name == "aime_archive":
        # Same shape as the aime recipe: skip __init__ (dataset download),
        # exercise task construction directly. The carve is eval-only --
        # AimeArchiveEnv.tasks() raises on split="train" -- so the split
        # under test is dev, as for amc.
        from infra.envs.tasks.aime_archive import AimeArchiveEnv
        from infra.envs.tasks.math import PROMPT_FILE

        env = object.__new__(AimeArchiveEnv)  # skip __init__ (dataset download)
        env.prompts = load_generation_prompts(resolve_prompt_file(None, PROMPT_FILE))
        row = {"problem": "What is 1+1?", "gt": 2.0, "level": 0, "id": "1983-1"}
        tasks = [AimeArchiveEnv._task(env, row, "dev")]
    elif name == "amc":
        # Same shape as the math recipe: skip __init__ (dataset download),
