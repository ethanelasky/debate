...
def test_qwen_replay_job_uses_verified_cache_smoke_and_raw_transport() -> None:
    ...
    assert job["outputs"] == [
        {
            "remote": "/workspace/debate/outputs/math_judge_pair_qwen_replay_single_reasoning",
            "local": "~/code/debate/outputs/math_judge_pair_qwen_replay_single_reasoning",
        }
    ]
    command = job["command"]
    assert "SOURCE_COMMIT=16e3df82a2ebb1e092a59470a6d226c0dbf9016f" in command
