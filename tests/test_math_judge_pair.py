...
def test_config_pins_models_caps_prompt_and_disjoint_local_endpoints() -> None:
    ...
    assert qwen["agents"]["judge"]["model_settings"]["sampling"]["train"]["frequency_penalty"] == 0.01
    assert qwen["agents"]["judge"]["model_settings"]["sampling"]["eval"]["presence_penalty"] == 1.5
    assert qwen["agents"]["judge"]["model_settings"]["sampling"]["eval"]["frequency_penalty"] == 0.01
...
def test_qwen_replay_job_uses_verified_cache_smoke_and_raw_transport() -> None:
    ...
    assert job["outputs"] == [
        {
            "remote": "/workspace/debate/outputs/math_judge_pair_qwen_replay_presence10",
            "local": "~/code/debate/outputs/math_judge_pair_qwen_replay_presence10",
        }
    ]
    command = job["command"]
    assert "SOURCE_COMMIT=baa90d8a4548e13f767b3d1a3b19cbd7f38dd67c" in command
...
def test_manifest_identity_pins_dataset_model_and_adapter_sources() -> None:
    ...
    assert identity["judge_generation_by_arm"]["qwen"]["frequency_penalty"] == 0.01
